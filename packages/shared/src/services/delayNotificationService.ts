...
/**
 * Browser notification channel - uses platform notification adapter
 */
class BrowserNotificationChannel implements NotificationChannel {
  private permission: NotificationPermission = 'default';
  private notificationAdapter: INotificationAdapter | null = null;
  ...
  private getNotifications(): INotificationAdapter {
    if (!this.notificationAdapter) {
      this.notificationAdapter = PlatformContainer.get<INotificationAdapter>(PLATFORM_TOKENS.NOTIFICATIONS);
    }
    ...
    try {
      ...
      await notifications.showNotification(
        `🚀 ${notification.launch.name}`,
        {
          ...
          icon: '/logo192.png',
          tag: `launch-${notification.launch.id}`,
  ...
  private async checkPermission(): Promise<void> {
    try {
      const notifications = this.getNotifications();
      this.permission = await notifications.getPermissionStatus();

      if (this.permission === 'default') {
        this.permission = await notifications.requestPermission();
      }
    } catch (error) {
      console.error('[BrowserNotification] Failed to check permission:', error);
