...
export class LaunchDataService {
  ...
  constructor() {
    ...
    // Don't call initialization here - wait for first use
    // This prevents accessing storage before platform adapters are registered
  }
  ...
  private async ensureInitialized(): Promise<void> {
    ...
    await this.clearStaleDataOnStartup();
   ...
   */
  private async clearStaleDataOnStartup(): Promise<void> {
    const cachedLaunches = await launchDatabase.getAllLaunches();
    const now = new Date();
  ...
  async getLaunches(): Promise<Launch[]> {
    // Ensure service is initialized before accessing data
  ...
  private async fetchLaunches(): Promise<void> {
    ...
    try {
      ...
      const scheduleChanges = await launchDelayDetectionService.detectScheduleChanges(enrichedLaunches);
      if (scheduleChanges.length > 0) {
        console.log(`[LaunchDataService] Detected ${scheduleChanges.length} schedule changes`);
      }
      
      // Store in database
      for (const launch of enrichedLaunches) {
        await launchDatabase.saveLaunch(launch, 'FlightClub Enhanced Service');
      }

      // Clean up old launches
      await launchDatabase.cleanup();
      ...
      const dbLaunches = await launchDatabase.getAllLaunches();
  ...
  private async loadFromDatabaseFirst(): Promise<void> {
    ...
    if (cachedLaunches.length > 0) {
      ...
      const launchesNeedingUpdate = await launchDatabase.getLaunchesNeedingUpdate();
