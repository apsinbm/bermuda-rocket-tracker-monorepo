...
export class LaunchDatabase {
   ...
   */
  async getLaunchesNeedingUpdate(): Promise<LaunchCacheEntry[]> {
    const entries = await this.getAllEntries();
    const now = Date.now();
   ...
   */
  async saveLaunch(launch: Launch, sourceUrl: string): Promise<void> {
    const entries = await this.getAllEntries();
  ...
  async cleanup(): Promise<void> {
    ...
    await this.saveAllEntries(cleanedEntries);
    await this.updateMetadata();
  }
  ...
  async getStats(): Promise<{
    ...
    const needingUpdate = await this.getLaunchesNeedingUpdate();
  ...
  async clear(): Promise<void> {
    ...
    await storage.removeItem(this.METADATA_KEY);
  }
  ...
  public async clearCache(): Promise<void> {
    ...
    await storage.setItem(this.METADATA_KEY, JSON.stringify(updatedMetadata));
  }

  /**
   * Get all cache entries (for schedule change detection)
   */
  public async getAllEntries(): Promise<LaunchCacheEntry[]> {
    try {
      const storage = this.getStorage();
  ...
  private async saveAllEntries(entries: LaunchCacheEntry[]): Promise<void> {
    try {
      const storage = this.getStorage();
      await storage.setItem(this.STORAGE_KEY, JSON.stringify(entries));
    } catch (error) {
      console.error('Failed to save launch database:', error);
    }
  }

  private async getMetadata(): Promise<DatabaseMetadata> {
    try {
      const storage = this.getStorage();
      const data = await storage.getItem(this.METADATA_KEY);
      return data ? JSON.parse(data) : {
        lastFullScan: 0,
        totalLaunches: 0,
  ...
  private async updateMetadata(): Promise<void> {
    ...
    try {
      const storage = this.getStorage();
      await storage.setItem(this.METADATA_KEY, JSON.stringify(metadata));
    } catch (error) {
